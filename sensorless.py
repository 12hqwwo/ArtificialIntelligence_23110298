...
class SensorlessApp(QMainWindow):
    def __init__(self):
        ...
        # Áp style cho tất cả QLabel trong beliefState
        for board_idx in range(1, 4):
            for i in range(3):
                for j in range(4):  # đúng theo số cột trong UI sensorless
                    name = f"label_{board_idx}_{i}_{j}"
        ...
        """
        for btn_name in ["randomButton", "solveButton", "exitButton", "resetButton"]:
